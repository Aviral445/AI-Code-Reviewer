  ...
  const handleReview = async () => {
    ...
    try {
      ...
      if (err?.response) {
        ...
        setErrorMsg("Failed to connect to backend (http://localhost:8000). Please make sure the backend is running.");
      }
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-base font-bold tracking-tight text-white">AI Code Reviewer</h1>
            ...
            <p className="text-xs text-slate-400 hidden sm:block">
              Static linter checks & Groq-powered AI code analysis
            </p>
          </div>
        </div>
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-4 sm:p-6 lg:p-8 flex flex-col gap-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
          ...
          <div className="flex flex-col bg-slate-900/60 border border-slate-800/80 rounded-2xl overflow-hidden shadow-xl">
            <div className="px-4 py-2.5 bg-slate-900/90 border-b border-slate-800/80 flex items-center justify-between text-xs text-slate-400">
              <div className="flex items-center gap-2 font-mono">
                ...
                <span className="text-slate-300 font-medium">Input Code</span>
              </div>
            ...
            <div className="relative">
              <textarea
                ...
                placeholder={`// Paste your Python, JavaScript, TypeScript, or other code here...\n// Press Ctrl+Enter (or Cmd+Enter) to run analysis`}
                className="w-full h-[450px] p-4 bg-slate-950 font-mono text-xs text-slate-200 resize-none focus:outline-none placeholder:text-slate-600 leading-relaxed"
                spellCheck="false"
              />
            </div>
          </div>
          ...
          <div className="flex flex-col bg-slate-900/60 border border-slate-800/80 rounded-2xl overflow-hidden shadow-xl min-h-[495px]">
            <div className="px-4 py-2.5 bg-slate-900/90 border-b border-slate-800/80 flex items-center justify-between text-xs text-slate-400">
              <div className="flex items-center gap-2">
                <Terminal className="w-3.5 h-3.5 text-amber-400" />
                <span className="text-slate-300 font-medium">Analysis Report</span>
              </div>
              {review && (
                <button
              ...
              {loading ? (
                <div className="h-full flex flex-col items-center justify-center gap-3 py-16 text-slate-500">
                  ...
                  <p className="text-xs font-mono animate-pulse text-indigo-300">
                    Running static analysis & AI evaluation...
                  </p>
                </div>
              ) : review ? (
                ...
                <div className="h-full flex flex-col items-center justify-center text-center py-20 px-4 text-slate-600">
                  <Sparkles className="w-8 h-8 mb-3 text-slate-700" />
                  <p className="text-xs font-medium text-slate-400">Ready for review</p>
                  <p className="text-[11px] text-slate-600 mt-1 max-w-xs">
                    Paste your code on the left or select a template, then click <strong>Review Code</strong>.
                  </p>
                </div>
              )}
            </div>
